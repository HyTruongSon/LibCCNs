...
class Dataset:
	...
	def reversed_edges_to_tensor(self):
		...
		self.reversed_edges_tensor = tf.convert_to_tensor(value = self.reversed_edges_tensor, dtype = dtype_int)
	...
	def set_mask(self, split):
		result = [False for index in range(self.nPapers)]
		for example in range(len(split)):
			index = split[example]
			result[index] = True
	...
	def to_one_hot(self, labels):
		result = np.zeros([len(labels), self.nClasses])
		for example in range(len(labels)):
			result[example, labels[example]] = 1.0
		return result
	...
	def set_labels_and_masks(self):
		...
		self.train_label = self.to_one_hot(self.train_label)
		self.val_label = self.to_one_hot(self.val_label)
		self.test_label = self.to_one_hot(self.test_label)
	...
	def sparse_adj(self):
		...
		self.sparse_adj = tf.SparseTensor(indices = indices_tensor, values = values_tensor, dense_shape = [self.nPapers, self.nPapers])
	...
	def sparse_symm_adj(self):
		...
		indices_tensor = np.concatenate((x, y), axis = 1)
		values_tensor = np.array(v)
		self.sparse_symm_adj = tf.SparseTensor(indices = indices_tensor, values = values_tensor, dense_shape = [self.nPapers, self.nPapers])
	...
	def sparse_feature(self):
		...
		indices_tensor = np.concatenate((x, y), axis = 1)
		values_tensor = np.array(v)
		self.sparse_feature = tf.SparseTensor(indices = indices_tensor, values = values_tensor, dense_shape = [self.nPapers, self.nVocab])
