...
flags.DEFINE_integer('max_nVertices_in_batch', 1000, 'Maximum number of vertices in a batch')
...
def train_batch(data, indices, model):
	# Batch creation
	batch = []
	...
	graph = Graph.Graph(batch, data.all_atomic_type, data.nClasses)

	# Training
	costs = model.train(graph)
...
def predict_batch(data, indices, model):
	...
	# Predicting
	accuracy = model.get_accuracy(graph)
...
def get_batch(data, indices, model):
	...
	# Predicting
	return model.get_representation(graph)	
...
def main(argv):
	...
	tsne = FLAGS.tsne
	fold = FLAGS.fold
	max_nVertices_in_batch = FLAGS.max_nVertices_in_batch
	...
	for epoch in range(epochs):
		...
		while start < nTrain:
			...
			loss = train_batch(data, data.train_indices[start:finish], model)
			avg_loss += loss
			nBatch += 1
			start = finish
		avg_loss /= nBatch
		...
		while start < nTest:
			...
			correct += predict_batch(data, data.test_indices[start:finish], model)
			start = finish
		accuracy = 100.0 * correct / nTest		
		end_time = time.time()
		...
		LOG(report_fn, 'Testing accuracy = ' + str(accuracy))
